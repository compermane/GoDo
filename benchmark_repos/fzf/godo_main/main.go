...
var tempo string = "1min"
var retest_indices []int = []int{8}
...
func main() {
	...
	for i < len(retest_indices) {
		...
		cmd := exec.Command("go", "test", "-timeout", "30m",  fmt.Sprintf("-memprofile=./%v_run_info/pprof/mem-%v.prof", tempo, index),
							fmt.Sprintf("-coverprofile=../godo_coverages/%v_runs/coverage-%v.out", tempo, index), "-coverpkg=../src,../src/algo,../src/tui,../src/util",
		...
		if err != nil {
			fmt.Println("Erro ao executar teste:", err.Error())
			error_counter++
		} else {
