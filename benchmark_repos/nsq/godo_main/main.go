...
var tempo string = "10min"
var retest_indices []int = []int{3, 27}
...
func main() {
	...
	for i < len(retest_indices) {
		...
		cmd := exec.Command("go", "test", "-timeout", "30m",  fmt.Sprintf("-memprofile=./%v_run_info/pprof/mem-%v.prof", tempo, index),
							fmt.Sprintf("-coverprofile=../godo_coverages/%v_runs/coverage-%v.out",tempo, index), "-coverpkg=../internal/app,../internal/auth,../internal/clusterinfo,../internal/dirlock,../internal/http_api,../internal/lg,../internal/pqueue,../internal/protocol,../internal/quantile,../internal/statsd,../internal/stringy,../internal/util,../internal/writers,../nsqadmin,../nsqd,../nsqlookupd",
