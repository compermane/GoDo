...
var retest_indices []int = []int{23, 10}
...
func main() {
	...
	for i < len(retest_indices) {
		...
		cmd := exec.Command("go", "test", "-timeout", "30m",  fmt.Sprintf("-memprofile=./%v_run_info/pprof/mem-%v.prof",tempo, index),
							fmt.Sprintf("-coverprofile=../godo_coverages/%v_runs/coverage-%v.out", tempo, index), "-coverpkg=../,../doc",
	...
	fmt.Fprintf(f, "Para a execução de 3 runs de %v, ocorreram %v erros\n", tempo, error_counter)
}