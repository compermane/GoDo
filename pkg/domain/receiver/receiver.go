...
func (rcv *Receiver) SetReceiverValues(rcvs_list []*Receiver) {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if rcv.AttrTypes[i].Kind().String() == "func" {
			...
			default:
				fmt.Println(rcv.AttrTypes[i].Kind())
				if rcv.AttrTypes[i].Kind() == reflect.Slice {
					t := utils.GetSliceType(rcv.AttrTypes[i])
					slice_field := reflect.SliceOf(t)
					arr_value    = reflect.MakeSlice(slice_field, 0, 0).Interface().([]any)

				} else if rcv.AttrTypes[i].Kind() == reflect.Func {
...
func proccess_receiver(struct_type reflect.Type, rcv any) *Receiver {
	...
	return InitReceiver(rcv, struct_type.String(),  methods, method_names, attr_names, attr_types, false)
}
...
func process_star_receiver(struct_type reflect.Type, rcv any) *Receiver {
	...
	return InitReceiver(rcv, struct_type.String(), methods, method_names, attr_names, attr_types, true)
}
